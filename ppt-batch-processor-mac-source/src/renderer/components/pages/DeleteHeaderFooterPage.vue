<template>
  <FeaturePage
    :title="t('删除页眉页脚')"
    :description="t('移除PPT文件中的页眉和页脚内容')"
    icon="📑"
    :can-start="canStart"
    :hint="t('请先选择文件')"
    @back="$emit('back')"
  ...
  >
    <template #config>
      <div class="config-section">
        <div class="info-box">
          <strong>{{ t('删除页眉页脚') }}</strong>
          <p>{{ t('移除PPT文件中的页眉和页脚内容，包括页码、日期、文本等') }}</p>
        </div>

        <div class="form-group">
          <label class="form-label">{{ t('删除选项') }}</label>
          <div class="checkbox-grid">
            <label class="checkbox-item">
              <input type="checkbox" v-model="options.removeHeader" />
              <span>{{ t('删除页眉') }}</span>
            </label>
            <label class="checkbox-item">
              <input type="checkbox" v-model="options.removeFooter" />
              <span>{{ t('删除页脚') }}</span>
            </label>
            <label class="checkbox-item">
              <input type="checkbox" v-model="options.removeSlideNumber" />
              <span>{{ t('删除页码') }}</span>
            </label>
            <label class="checkbox-item">
              <input type="checkbox" v-model="options.removeDateTime" />
              <span>{{ t('删除日期时间') }}</span>
            </label>
          </div>
        </div>
