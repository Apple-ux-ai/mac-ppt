...
import { AppError, ErrorCode, createError } from '../utils/error-codes'
import { progressTracker, ProgressStage } from '../utils/progress-tracker'
import { taskPersistence, PersistedTask, TaskFileInfo } from '../utils/task-persistence'
import {
  extractLocalizedError,
  createLocalizedError,
  createLocalizedErrorPayload,
} from '../utils/user-visible-messages'
...
export async function registerIPCHandlers(): Promise<void> {
  ...
  ipcMain.handle('get-file-stats', (_event, filePath: string) => handleGetFileStats(filePath))
  ipcMain.handle('get-image-list', (_event, folderPath: string) => handleGetImageList(folderPath))
  ipcMain.handle('process-files', (_event, taskConfig: any) => {
  ...
  ipcMain.handle('cancel-task', (_event, taskId: string) => handleCancelTask(taskId))
  ipcMain.handle('download-file', (_event, filePath: string) => handleDownloadFile(filePath))
  ipcMain.handle('download-all-files', (_event, filePaths: string[]) =>
    handleDownloadAllFiles(filePaths)
  )

  ipcMain.handle('validate-files', (_event, filePaths: string[], options?: any) =>
    handleValidateFiles(filePaths, options)
  )
  ipcMain.handle('get-task-progress', (_event, taskId: string) => handleGetTaskProgress(taskId))
  ipcMain.handle('get-resumable-tasks', handleGetResumableTasks)
  ipcMain.handle('resume-task', (_event, taskId: string) => handleResumeTask(taskId))
  ipcMain.handle('get-task-history', handleGetTaskHistory)
  ipcMain.handle('export-task-history', (_event, outputPath: string, format?: 'json' | 'csv') =>
    handleExportTaskHistory(outputPath, format)
  )
...
async function handleSelectFiles(): Promise<{
  files: string[]
  ...
  const errors: Array<{ file: string; errors: any[]; warnings: any[] }> = []
...
async function handleSelectExcelFile(): Promise<string | null> {
...
async function handleSelectImageFile(): Promise<string | null> {
...
async function handleGetFileStats(filePath: string): Promise<any> {
  try {
...
async function handleGetImageList(folderPath: string): Promise<any[]> {
  try {
...
async function handleProcessFiles(
  ...
  try {
    ...
    for (let i = 0; i < totalFiles; i++) {
      ...
      progressTracker.setStage(
        taskId,
        ProgressStage.PROCESSING,
        `正在处理: ${path.basename(inputPath)}`,
        'main.progress.processingFile',
      ...
      try {
        switch (options.taskType) {
          ...
          case 'template-generate': {
            ...
            log(
              `✓ Template generate: ${path.basename(inputPath)} -> ${generatedFiles.length} files`
            )
            continue
          }
          case 'image-replace':
        ...
        if (errorStack) log(`Stack trace: ${errorStack}`)
    ...
    if (options.taskType === 'merge-ppt' && actualInputFiles.length > 0) {
      ...
      try {
        const actualOutputPath = await processMergePpt(
          firstFile.path,
          defaultOutputPath,
          options,
          actualInputFiles
    ...
    const finalOutputDir =
      outputDir || path.join(require('os').homedir(), 'Downloads', 'ppt-processor')
    ...
    for (const result of results) {
      ...
      if (result.status === 'success' && result.outputPath && !result.skipMove) {
        ...
        try {
          ...
          log(
            `  ✗ Failed to ${overwriteOriginal ? 'overwrite' : 'move'} file: ${path.basename(result.outputPath)}. Error: ${moveErrorMsg}`
          )
        }
...
async function handleCancelTask(taskId: string): Promise<boolean> {
...
async function processReplaceText(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
...
async function processDeleteText(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
...
async function processBatchReplace(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
...
async function processDeletePages(
  ...
  if (deleteMode === 'range') {
    ...
    const parts = pageRange.split(',').map((s: string) => s.trim())

    for (const part of parts) {
      if (part.includes('-')) {
        const [start, end] = part.split('-').map((s: string) => parseInt(s.trim()))
    ...
    if (deletePages.length === 0) {
      ...
      for (let i = 1; i <= deletePages.length; i++) {
        if (i < deletePages.length && deletePages[i] === rangeEnd + 1) {
          ...
          if (deleteRangeStr) deleteRangeStr += ','
...
async function processReplacePages(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
...
async function processInsertPages(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
  ...
  console.log(
    `    Inserting pages from ${options.insertSourcePageRange} at position ${options.insertPosition}`
  )
...
async function processOptimize(inputPath: string, outputPath: string, options: any): Promise<void> {
...
export async function processSplitPpt(
  inputPath: string,
  outputDir: string,
  options: any
): Promise<string[]> {
...
async function processMergePpt(
  inputPath: string,
  outputPath: string,
  options: any,
  inputFiles: any[]
  ...
  // 获取所有输入文件路径
  const filePaths = inputFiles.map((f: any) => f.path)
 ...
 */
async function validateAndRepairWithLibreOffice(filePath: string): Promise<void> {
  try {
    ...
    try {
      ...
      await execFileAsync(
        libreOfficePath,
        ['--headless', '--convert-to', 'pptx', '--outdir', tempDir, filePath],
        { timeout: 30000 }
      )
...
async function processAddWatermark(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
...
async function processFormatConvert(
  inputPath: string,
  outputDir: string,
  options: any
  ...
  if (targetFormat === 'pdf') {
    ...
    const fsNode: any = await import('fs')
    ...
    console.log(
      `    Image conversion completed: ${imageFiles.length} files, zipped to ${zipFilePath}`
    )
    return zipFilePath
...
async function processExtractPages(
  ...
  // 解析页码范围（例如 "1-3,5,7-9"）
  const ranges = options.extractPageRange.split(',').map((r: string) => r.trim())
  let startPage = 1
  let endPage = 1
  ...
  if (ranges.length === 1) {
    ...
    // 这里暂时只支持简单的单个范围
    throw new Error(
      'Complex page ranges not yet supported. Please use a single range like "1-3" or a single page like "5"'
    )
  }
...
async function processExtractImages(
  ...
  try {
    ...
    console.log(
      `    Images extracted successfully: ${extractedImages.length} images, zipped to ${zipFilePath}`
    )
    return zipFilePath
  } finally {
...
async function processExtractNotes(
  inputPath: string,
  outputDir: string,
  options: any
): Promise<string> {
...
async function processEditMetadata(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
  ...
  // 构建元数据对象
  const metadata: any = {}
  if (options.metadataTitle) metadata.title = options.metadataTitle
  if (options.metadataAuthor) metadata.author = options.metadataAuthor
  if (options.metadataSubject) metadata.subject = options.metadataSubject
  if (options.metadataKeywords) metadata.keywords = options.metadataKeywords
  if (options.metadataComments) metadata.comments = options.metadataComments
...
async function processClearMetadata(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
...
async function processAddPassword(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
...
async function processRemovePassword(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
...
async function processOptimizeCompress(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
...
async function processDeleteNotes(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
...
async function processDeleteBackground(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
  ...
  console.log(
    `    Background deleted from ${result.processedSlides} slides, ${result.processedMasters} masters`
  )
}

async function processDeleteHeaderFooter(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
  ...
  console.log(
    `    Header/footer deleted from ${result.processedSlides} slides, removed ${result.removedCount} items`
  )
}

async function processDeleteMacros(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
...
async function processTextReplace(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
...
async function processExcelRules(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
  ...
  if (result.status === 'error') {
    ...
    throw createLocalizedError(
      'main.error.ruleApplyFailed',
      `规则应用失败: ${result.error || '应用Excel规则失败'}`,
      { message: result.error || '应用Excel规则失败' }
    )
  }
...
async function processTemplateGenerate(
  inputPath: string,
  ...
  const outputFiles = await engine.batchGenerate(
    template,
    dataList,
    outputDir,
...
async function processImageReplace(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
...
async function processDeleteBlankPages(
  inputPath: string,
  outputPath: string,
  options: any
): Promise<void> {
  ...
  console.log(
    `    Deleted ${result.removedCount} blank pages, ${result.remainingCount} pages remaining`
  )
}
...
async function handleValidateFiles(
  filePaths: string[],
  options?: any
): Promise<{
  valid: boolean
...
async function handleGetTaskProgress(taskId: string): Promise<any> {
  return progressTracker.getProgress(taskId)
}

async function handleGetResumableTasks(): Promise<PersistedTask[]> {
  return taskPersistence.getResumableTasks()
}

async function handleResumeTask(taskId: string): Promise<{
  success: boolean
  task?: PersistedTask
  resumeFrom?: number
  ...
  try {
    ...
    if (!result) {
      return {
        success: false,
        ...createLocalizedErrorPayload(
          'main.error.taskNotFoundOrNotResumable',
          getMainText('main.error.taskNotFoundOrNotResumable', '任务不存在或不可恢复')
        ),
      }
    }
...
async function handleGetTaskHistory(): Promise<PersistedTask[]> {
  return taskPersistence.getAllTasks()
}

async function handleExportTaskHistory(
  outputPath: string,
