 ...
 * 删除页面处理
 */
export async function processDeletePages(inputPath, outputPath, options) {
    ...
    // 执行删除
    const deletedCount = await processor.deletePages(inputPath, outputPath, options.pageRange);
    console.log(`    Deleted ${deletedCount} page(s)`);
}
/**
 * 替换页面处理
 */
export async function processReplacePages(inputPath, outputPath, options) {
    ...
    // 执行替换
    const replacedCount = await processor.replacePages(options.replaceTargetFile, options.replaceSourceFile, String(options.replacePageNumber), String(options.replaceSourcePageNumber), correctOutputPath);
 ...
 * 插入页面处理
 */
export async function processInsertPages(inputPath, outputPath, options) {
    ...
    console.log(`    Corrected output path: ${correctOutputPath}`);
    // 执行插入
    const insertedCount = await processor.insertPages(options.insertTargetFile, options.insertSourceFile, options.insertPosition, options.insertSourcePageRange, correctOutputPath);
