export function createLocalizedError(messageKey, fallback, messageParams) {
    const error = new Error(fallback);
    error.messageKey = messageKey;
    error.messageParams = messageParams;
    return error;
}
export function createLocalizedMessage(messageKey, fallback, messageParams) {
    return {
        message: fallback,
        messageKey,
        messageParams,
...
export function createLocalizedErrorPayload(errorKey, fallback, errorParams) {
    return {
        error: fallback,
        errorKey,
        errorParams,
...
function parseMessage(message) {
    ...
    match = message.match(/^LibreOffice 未安装或不可用。(.+)需要 LibreOffice。\n应用已内置 LibreOffice，如果此错误持续出现，请联系技术支持。$/);
    if (match) {
...
export function extractLocalizedError(error) {
    if (error instanceof Error) {
        ...
        if (localizedError.messageKey) {
            return createLocalizedErrorPayload(localizedError.messageKey, localizedError.message, localizedError.messageParams);
        }
