...
export class OutputPathGenerator {
   ...
   * @returns 文件信息
   */
  private parseInputPath(inputPath: string): FilePathInfo {
   ...
   * @returns 应用模式后的文件名
   */
  private applyPattern(pattern: string, info: FilePathInfo & { index?: number, total?: number }): string {
    const now = new Date()
   ...
   * @returns 占位符说明
   */
  getSupportedPlaceholders(): PlaceholderInfo[] {
    return [
      {
        placeholder: '{name}',
