...
export class FileBackupManager {
  ...
  // 备份记录：原文件路径 -> 备份文件路径
  private backupRegistry: Map<string, BackupInfo> = new Map()
  
  // 默认备份目录名称
  private static readonly DEFAULT_BACKUP_DIR = '.ppt-batch-processor-backups'
  
  constructor(customBackupDir?: string) {
    // 使用自定义备份目录或默认备份目录（在用户主目录下）
    this.backupDir = customBackupDir || path.join(
      process.env.USERPROFILE || process.env.HOME || '.',
      FileBackupManager.DEFAULT_BACKUP_DIR
    )
  }
   ...
   * @returns 备份信息数组
   */
  async backupFiles(filePaths: string[]): Promise<BackupInfo[]> {
   ...
   * @returns 恢复结果数组
   */
  async restoreFiles(filePaths: string[]): Promise<RestoreResult[]> {
   ...
   * 清理整个备份目录
   */
  async cleanupBackupDir(): Promise<void> {
    try {
      // 先清理注册的备份
   ...
   * @param days - 保留天数
   */
  async cleanupOldBackups(days: number = 7): Promise<number> {
    const now = Date.now()
