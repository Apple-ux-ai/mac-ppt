...
describe('OutputPathGenerator - Property Tests: Conflict Avoidance', () => {
    ...
    it('should generate unique paths that do not overwrite existing files', async () => {
        ...
        // 生成已存在文件的数量（0-10）
        fc.integer({ min: 0, max: 10 }), async (fileName, ext, existingCount) => {
            // 清理测试目录
    ...
    it('should generate all unique paths in batch processing', async () => {
        ...
        }), { minLength: 1, maxLength: 20 }), async (fileInfos) => {
            // 清理测试目录
            const files = await fs.readdir(testDir);
            for (const file of files) {
                await fs.unlink(path.join(testDir, file));
    ...
    it('should generate monotonically increasing sequence numbers for conflicts', async () => {
        ...
        // 生成重复次数（2-10）
        fc.integer({ min: 2, max: 10 }), async (fileName, ext, repeatCount) => {
            // 清理测试目录
            const files = await fs.readdir(testDir);
            for (const file of files) {
                await fs.unlink(path.join(testDir, file));
    ...
    it('should not modify existing files when resolving conflicts', async () => {
        ...
        // 生成文件内容
        fc.array(fc.string({ minLength: 1, maxLength: 100 }), { minLength: 1, maxLength: 5 }), async (fileName, ext, existingCount, contents) => {
            // 清理测试目录
            const files = await fs.readdir(testDir);
            for (const file of files) {
                await fs.unlink(path.join(testDir, file));
            ...
            const inputPath = `/test/${fileName}${ext}`;
            // 创建已存在的文件并记录内容
            const existingFilesContent = new Map();
            for (let i = 0; i < existingCount; i++) {
    ...
    it('should avoid conflicts even with custom patterns', async () => {
        ...
        validStringArb, 
        // 生成已存在文件数量
        fc.integer({ min: 0, max: 3 }), async (fileName, ext, prefix, suffix, existingCount) => {
            // 清理测试目录
            const files = await fs.readdir(testDir);
            for (const file of files) {
                await fs.unlink(path.join(testDir, file));
    ...
    it('should preserve file extension when resolving conflicts', async () => {
        ...
        // 生成冲突数量
        fc.integer({ min: 0, max: 5 }), async (fileName, ext, conflictCount) => {
            // 清理测试目录
            const files = await fs.readdir(testDir);
            for (const file of files) {
                await fs.unlink(path.join(testDir, file));
    ...
    it('should handle mix of existing and new files in batch processing', async () => {
        ...
        }), { minLength: 2, maxLength: 10 }), async (fileInfos) => {
            // 清理测试目录
            const files = await fs.readdir(testDir);
            for (const file of files) {
                await fs.unlink(path.join(testDir, file));
