 ...
 * 删除结果
 */
export interface RemoveNotesResult {
  success: boolean
...
export class NotesRemover {
  ...
  async removeNotes(
    inputPath: string,
    outputPath: string
  ): Promise<RemoveNotesResult> {
    const result: RemoveNotesResult = {
      success: true,
      processedSlides: 0,
      errors: []
    }
   ...
   */
  private async removeSlideNotes(zip: any, slideNumber: number): Promise<void> {
