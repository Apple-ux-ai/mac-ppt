...
export class PptxParser {
    ...
    replaceInTextNodes(xml, searchText, replaceText, useRegex) {
        let totalCount = 0;
    ...
    replaceTextInShape(shapeXml, searchText, replaceText, useRegex) {
        const txBodyMatch = shapeXml.match(/<p:txBody>([\s\S]*)<\/p:txBody>/);
        if (!txBodyMatch)
            return { xml: shapeXml, count: 0 };
    ...
    async replaceTextInSlide(document, slideIndex, searchText, replaceText) {
        ...
        try {
            ...
            if (error instanceof Error) {
                ...
                if (error.message.includes('EBUSY') || error.message.includes('resource busy')) {
                    ...
                    throw new Error(`文件被占用：${fileName}\n\n` +
                        `可能的原因：\n` +
                        `• 文件在PowerPoint中打开\n` +
                        `• 文件在其他程序中使用\n` +
                        `• 文件被系统锁定\n\n` +
     ...
     */
    async getImagesWithData(document, slide, slideIndex) {
        const zip = document.zipArchive;
        const imageElements = [];
        // 遍历幻灯片中的所有图片元素
        for (const element of slide.elements) {
     ...
     * @returns 删除的元素数量
     */
    async deleteElement(document, slide, element) {
        try {
            const zip = document.zipArchive;
            // 查找幻灯片索引
            ...
            if (element) {
                if (element.type === 'image') {
                    const deleted = await this.deleteImageElement(slideData, element);
                    if (deleted) {
                        deletedCount = 1;
                        // 从幻灯片的元素列表中移除
     ...
     * 从幻灯片 XML 中删除图片元素
     */
    async deleteImageElement(slideData, element) {
        try {
     ...
     * 从组合形状中删除图片元素
     */
    async deleteImageFromGroup(grpSp, element) {
        try {
            // 在组合中查找并删除图片
     ...
     */
    isMatchingImageElement(pic, element) {
        try {
    ...
    async parseMetadata(zip) {
        try {
            ...
            if (props) {
                ...
                if (props['dc:description'])
                    metadata.description = props['dc:description'][0];
                if (props['cp:keywords'])
    ...
    async parseSlides(zip) {
        ...
        try {
            // 获取所有幻灯片文件
            const slideFiles = Object.keys(zip.files).filter(name => name.startsWith('ppt/slides/slide') && name.endsWith('.xml') && !name.includes('_rels'));
            // 按编号排序
            slideFiles.sort((a, b) => {
     ...
     * 解析单个幻灯片
     */
    async parseSlide(zip, slidePath) {
        try {
     ...
     * 更新元数据
     */
    async updateMetadata(zip, metadata) {
        try {
            const corePropsFile = zip.file('docProps/core.xml');
            if (!corePropsFile)
                return;
            ...
            if (metadata.title !== undefined)
                props['dc:title'] = [metadata.title];
            if (metadata.subject !== undefined)
                props['dc:subject'] = [metadata.subject];
            if (metadata.author !== undefined)
                props['dc:creator'] = [metadata.author];
            if (metadata.description !== undefined)
                props['dc:description'] = [metadata.description];
            if (metadata.keywords !== undefined)
                props['cp:keywords'] = [metadata.keywords];
            if (metadata.comments !== undefined)
                props['dc:description'] = [metadata.comments];
            // 构建新的 XML
            const builder = new Builder({
                xmldec: { version: '1.0', encoding: 'UTF-8', standalone: true }
            });
     ...
     * 替换文本
     */
    async replaceText(document, element, searchText, replaceText) {
        try {
            const slideIndex = element?.slideIndex;
    ...
    async replaceImage(document, slideIndex, imageElement, newImageData, newImageExt, keepOriginalSize = true, replaceAllSameImages = false) {
     ...
     */
    async updateImageRelationship(zip, slideIndex, relationshipId, newImageName) {
        ...
        const relationshipRegex = new RegExp(`(<Relationship[^>]*Id="${relationshipId}"[^>]*Target=")([^"]+)("[^>]*/>)`, 'i');
     ...
     */
    findImagePath(zip, slideIndex, relationshipId) {
        ...
        try {
            ...
            // 解析关系文件，查找对应的图片路径
            const relationshipRegex = new RegExp(`<Relationship[^>]*Id="${relationshipId}"[^>]*Type="[^"]*image"[^>]*Target="([^"]+)"[^>]*/>`, 'i');
            let match = relationshipRegex.exec(relsContent);
            // 如果没有找到带 Type="image" 的，尝试不限制类型但检查扩展名
            if (!match) {
                const anyRelRegex = new RegExp(`<Relationship[^>]*Id="${relationshipId}"[^>]*Target="([^"]+)"[^>]*/>`, 'i');
