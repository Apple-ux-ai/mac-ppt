...
export class MetadataEditor {
   ...
   */
  async read(filePath: string): Promise<Metadata> {
   ...
   */
  async write(filePath: string, metadata: Metadata): Promise<void> {
    const document = await this.parser.open(filePath)
   ...
   * @returns 成功处理的文件数量
   */
  async batchUpdate(filePaths: string[], metadata: Partial<Metadata>): Promise<number> {
    let successCount = 0
   ...
   */
  async editMetadata(inputPath: string, outputPath: string, metadata: Partial<Metadata>): Promise<void> {
    try {
   ...
   * @param outputPath 输出文件路径
   */
  async clearMetadata(inputPath: string, outputPath: string): Promise<void> {
    try {
      const document = await this.parser.open(inputPath)
