...
export class BlankPageRemover {
     ...
     * @returns 删除结果
     */
    async removeBlankPages(inputPath, outputPath, options) {
        const result = {
            success: true,
            removedCount: 0,
            remainingCount: 0,
    ...
    isBlankSlide(slide, options) {
        ...
        const hasShapeContent = slide.elements.some(element => {
            if (element.type === 'text' || element.type === 'image')
                return false;
            const el = element;
            if (el.placeholder || el.isPlaceholder)
                return false;
            return true;
        });
     ...
     */
    async removeSlide(document, slideIndex) {
     ...
     */
    async updatePresentationRels(zip, slideIndex) {
        ...
        // 过滤掉要删除的幻灯片关系
        result.Relationships.Relationship = result.Relationships.Relationship.filter((rel) => {
     ...
     */
    async updatePresentationXml(zip, slideIndex) {
