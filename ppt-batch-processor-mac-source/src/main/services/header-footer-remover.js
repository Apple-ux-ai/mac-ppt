...
export class HeaderFooterRemover {
     ...
     * @returns 删除结果
     */
    async removeHeaderFooter(inputPath, outputPath, options) {
        const result = {
            success: true,
            processedSlides: 0,
            removedCount: 0,
     ...
     * @returns 删除的元素数量
     */
    async removeSlideHeaderFooter(zip, slideNumber, options) {
    ...
    removeHeaderFooterFromSpTree(spTree, options) {
        let removedCount = 0;
        const filterShapes = (node) => {
            if (!node)
                return;
    ...
    isLikelyHeaderFooterTextShape(shape, options) {
    ...
    async removeHeaderFooterFromMatchingParts(zip, pathPattern, options) {
        const files = zip.file(/.*/);
        const matched = files
            .map((f) => f.name)
            .filter((name) => pathPattern.test(name));
        let totalRemoved = 0;
    ...
    async removeHeaderFooterFromPart(zip, partPath, options) {
     ...
     * @returns 是否应该删除
     */
    isHeaderFooterShape(shape, options) {
        try {
            // 检查占位符类型
