 ...
 * 删除结果
 */
export interface RemoveMacrosResult {
  success: boolean
  hadMacros: boolean
  removedFiles: string[]
  errors: string[]
...
export class MacroRemover {
  ...
  async removeMacros(
    inputPath: string,
    outputPath: string
  ): Promise<RemoveMacrosResult> {
    const result: RemoveMacrosResult = {
      success: true,
      hadMacros: false,
      removedFiles: [],
      errors: []
   ...
   * @returns 是否包含宏
   */
  private checkForMacros(zip: any): boolean {
   ...
   * @param result 结果对象
   */
  private async removeVbaProjectFile(zip: any, result: RemoveMacrosResult): Promise<void> {
   ...
   * @param result 结果对象
   */
  private async updateContentTypes(zip: any, result: RemoveMacrosResult): Promise<void> {
    ...
    try {
      ...
      if (parsed.Types && parsed.Types.Override) {
        ...
        parsed.Types.Override = parsed.Types.Override.filter((override: any) => {
      ...
      if (parsed.Types && parsed.Types.Default) {
        ...
        parsed.Types.Default = parsed.Types.Default.filter((def: any) => {
   ...
   * @param result 结果对象
   */
  private async updateRootRels(zip: any, result: RemoveMacrosResult): Promise<void> {
