...
export class PageProcessor {
     ...
     * @throws Error 如果拆分失败
     */
    async splitPpt(inputPath, outputDir, pagesPerFile, fileNamePattern) {
        if (pagesPerFile <= 0) {
        ...
        try {
            ...
            for (let i = 0; i < fileCount; i++) {
                ...
                const outputFileName = this.generateFileName(baseName, i + 1, fileCount, fileNamePattern);
                const outputPath = path.join(outputDir, `${outputFileName}.pptx`);
                await this.createDocumentWithSlides(document, slidesForFile, outputPath);
     ...
     */
    generateFileName(baseName, index, total, pattern) {
        if (pattern) {
            return pattern
     ...
     */
    async createDocumentWithSlides(sourceDocument, slides, outputPath) {
        try {
            ...
            const sourceZip = sourceDocument.zipArchive;
     ...
     * @throws Error 如果合并失败
     */
    async mergePpt(inputPaths, outputPath) {
        ...
        try {
            ...
            const baseZip = baseDoc.zipArchive;
            ...
            for (let fileIdx = 1; fileIdx < inputPaths.length; fileIdx++) {
                ...
                const sourceZip = sourceDoc.zipArchive;
                ...
                const copyPartWithRels = async (partPath) => {
                ...
                for (let slideIdx = 0; slideIdx < sourceDoc.slides.length; slideIdx++) {
                    ...
                    try {
                        const themeFiles = Object.keys(sourceZip.files).filter(filePath => filePath.startsWith('ppt/theme/') && filePath.endsWith('.xml'));
                        ...
                        const masterFiles = Object.keys(sourceZip.files).filter(filePath => filePath.startsWith('ppt/slideMasters/') && filePath.endsWith('.xml'));
                        ...
                        const layoutFiles = Object.keys(sourceZip.files).filter(filePath => filePath.startsWith('ppt/slideLayouts/') && filePath.endsWith('.xml'));
                        ...
                        const relsFiles = Object.keys(sourceZip.files).filter(filePath => (filePath.startsWith('ppt/slideMasters/_rels/') ||
     ...
     */
    async rebuildPresentationForMerge(zip, parseXml, totalSlides) {
     ...
     */
    async rebuildPresentationRels(zip, parseXml, slideCount) {
        const Builder = (await import('xml2js')).Builder;
        ...
        const nonSlideRels = relationships.filter((rel) => {
            const type = rel.$?.Type || '';
     ...
     */
    async updateContentTypes(zip, parseXml, totalSlides) {
        const Builder = (await import('xml2js')).Builder;
        ...
        const existingPartNames = new Set(filteredOverrides.map((override) => override.$?.PartName).filter(Boolean));
        ...
        for (let i = 1; i <= totalSlides; i++) {
            addOverride(`/ppt/slides/slide${i}.xml`, 'application/vnd.openxmlformats-officedocument.presentationml.slide+xml');
            ...
            if (zip.file(notesPath)) {
                addOverride(`/ppt/notesSlides/notesSlide${i}.xml`, 'application/vnd.openxmlformats-officedocument.presentationml.notesSlide+xml');
        ...
        for (const filePath of slideMasterFiles) {
            addOverride(`/${filePath}`, 'application/vnd.openxmlformats-officedocument.presentationml.slideMaster+xml');
        }
        ...
        for (const filePath of slideLayoutFiles) {
            addOverride(`/${filePath}`, 'application/vnd.openxmlformats-officedocument.presentationml.slideLayout+xml');
        }
        const themeFiles = allFiles.filter(filePath => /^ppt\/theme\/theme\d+\.xml$/.test(filePath));
        for (const filePath of themeFiles) {
            addOverride(`/${filePath}`, 'application/vnd.openxmlformats-officedocument.theme+xml');
        }
        ...
        for (const filePath of notesMasterFiles) {
            addOverride(`/${filePath}`, 'application/vnd.openxmlformats-officedocument.presentationml.notesMaster+xml');
        }
        ...
        for (const filePath of handoutMasterFiles) {
            addOverride(`/${filePath}`, 'application/vnd.openxmlformats-officedocument.presentationml.handoutMaster+xml');
        }
     ...
     */
    async updatePresentationXmlForNewDocument(zip, totalSlides, parseXml) {
        const presentationFile = zip.file('ppt/presentation.xml');
        if (!presentationFile) {
            throw new Error('presentation.xml not found');
        }
        ...
        // 替换 sldIdLst 内容
        presentationXml = presentationXml.replace(/<p:sldIdLst[^>]*>[\s\S]*?<\/p:sldIdLst>/, `<p:sldIdLst>${newSlideIdsXml}</p:sldIdLst>`);
        zip.file('ppt/presentation.xml', presentationXml);
    }
    /**
     * 更新 presentation.xml.rels 中的幻灯片引用（用于新文档）
     */
    async updatePresentationRelsForNewDocument(zip, totalSlides, parseXml) {
        const relsPath = 'ppt/_rels/presentation.xml.rels';
        const relsFile = zip.file(relsPath);
        if (!relsFile) {
            throw new Error('presentation.xml.rels not found');
     ...
     */
    async updateContentTypesForNewDocument(zip, totalSlides, parseXml) {
        const contentTypesPath = '[Content_Types].xml';
        const contentTypesFile = zip.file(contentTypesPath);
        if (!contentTypesFile) {
            return;
     ...
     * @throws Error 如果提取失败
     */
    async extractPages(inputPath, outputPath, startPage, endPage) {
        try {
            const document = await this.parser.open(inputPath);
            const totalSlides = document.slides.length;
     ...
     * @returns 是否修复成功
     */
    async repairPpt(inputPath, outputPath) {
        try {
     ...
     * @throws Error 如果删除失败
     */
    async deletePages(inputPath, outputPath, pageRange) {
        try {
            const document = await this.parser.open(inputPath);
            const totalSlides = document.slides.length;
            // 解析页码范围
     ...
     */
    async replacePages(targetPath, sourcePath, targetPageRange, sourcePageRange, outputPath) {
        try {
            ...
            if (targetIndices.length !== sourceIndices.length) {
                throw new Error(`Page range mismatch: target has ${targetIndices.length} pages, source has ${sourceIndices.length} pages`);
            }
            // 获取源页面
            ...
            const PizZip = (await import('pizzip')).default;
            const sourceZip = sourceDoc.zipArchive;
            const targetZip = targetDoc.zipArchive;
     ...
     * @returns 插入的页面数量
     */
    async insertPages(targetPath, sourcePath, insertPosition, sourcePageRange, outputPath) {
        try {
     ...
     */
    async copyMediaFromSource(sourceDoc, targetDoc, sourceSlides) {
        try {
            const PizZip = (await import('pizzip')).default;
     ...
     */
    async updatePresentationXmlForInsert(document, insertPosition, insertCount) {
        try {
            const PizZip = (await import('pizzip')).default;
            const { parseString, Builder } = await import('xml2js');
            const { promisify } = await import('util');
            ...
            // 为新插入的幻灯片创建新的 ID 条目
            const newSlideIds = [];
            let maxId = Math.max(...allSlideIds.map((s) => parseInt(s.$?.id || '256')), 256);
     ...
     * @param totalSlides 总幻灯片数
     */
    async rebuildPresentationForInsert(zip, parseXml, totalSlides) {
        // 先重建 rels 文件，获取正确的 rId 映射
     ...
     * @returns rId 映射数组
     */
    async rebuildPresentationRelsForInsert(zip, parseXml, slideCount) {
        const Builder = (await import('xml2js')).Builder;
        const relsPath = 'ppt/_rels/presentation.xml.rels';
        const relsFile = zip.file(relsPath);
        if (!relsFile) {
