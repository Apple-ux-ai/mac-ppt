import type { Slide, WatermarkOptions, PptxDocument, ImageElement } from '../../shared/types'
...
export class WatermarkEngine {
  ...
  async addTextWatermark(
    ...
    try {
      ...
      // 创建文本框元素
      const textBoxElement = this.createTextBoxElement(
        options.content,
        position,
        options.fontSize || 36,
   ...
   * @returns 位置和尺寸信息
   */
  private calculateImageWatermarkPosition(
    position: WatermarkOptions['position'],
   ...
   * @returns 是否成功添加
   */
  private addElementToSlide(slideData: any, element: any): boolean {
    try {
   ...
   */
  private async getSlidePath(zip: PizZip, slideIndex: number): Promise<string | null> {
    try {
  ...
  async addWatermarkToImage(
    ...
    try {
      ...
      if (options.type === 'text') {
        // 创建文字水印
        watermarkBuffer = await this.createTextWatermarkImage(
          options.content,
          metadata.width,
          metadata.height,
          options.fontSize || 36,
        ...
        // 计算水印尺寸和位置
        const watermarkSize = this.calculateImageWatermarkSize(
          metadata.width,
          metadata.height,
          options.size,
        ...
        const watermarkPosition = this.calculateImageWatermarkPositionOnImage(
          metadata.width,
          metadata.height,
          watermarkSize.width,
   ...
   * @returns 水印图片 Buffer
   */
  private async createTextWatermarkImage(
    text: string,
    imageWidth: number,
    imageHeight: number,
    fontSize: number,
    ...
    // 计算文字位置
    const textPosition = this.calculateTextPositionOnImage(
      imageWidth,
      imageHeight,
      textWidth,
      textHeight,
   ...
   * @returns 水印尺寸
   */
  private calculateImageWatermarkSize(
    imageWidth: number,
    imageHeight: number,
   ...
   * @returns 水印位置坐标
   */
  private calculateImageWatermarkPositionOnImage(
    imageWidth: number,
    imageHeight: number,
    watermarkWidth: number,
    watermarkHeight: number,
   ...
   * @returns 文字位置坐标
   */
  private calculateTextPositionOnImage(
    imageWidth: number,
    imageHeight: number,
    textWidth: number,
    textHeight: number,
   ...
   */
  private getSharpGravity(position: WatermarkOptions['position']): string {
    switch (position) {
      case 'top-left':
        return 'northwest'
      case 'top-right':
