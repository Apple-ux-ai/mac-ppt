...
export class ImageReplacer {
     ...
     * @returns 替换结果
     */
    async replaceImages(inputPath, outputPath, rules) {
        const result = {
            success: true,
            replacedCount: 0,
            errors: []
        ...
        for (const slideIndex of slidesToProcess) {
            ...
            for (let i = 0; i < slide.elements.length; i++) {
                ...
                if (element.type === 'image') {
                    ...
                    // 替换图片
                    await this.replaceImage(document, slideIndex, imageElement, newImageData, newImageExt, rule.keepOriginalSize, rule.replaceAllSameImages);
                    replacedCount++;
    ...
    async replaceImage(document, slideIndex, imageElement, newImageData, newImageExt, keepOriginalSize = true, replaceAllSameImages = false) {
        // 使用 parser 的图片替换功能
        await this.parser.replaceImage(document, slideIndex, imageElement, newImageData, newImageExt, keepOriginalSize, replaceAllSameImages);
    }
    /**
     * 获取 PPT 中的所有图片信息
