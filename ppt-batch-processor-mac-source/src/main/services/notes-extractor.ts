...
export class NotesExtractor {
  ...
  async extractNotes(
    ...
    try {
      ...
      // 提取每个备注
      const notes: Array<{ slideNumber: number; content: string }> = []
   ...
   * 从备注 XML 中提取文本内容
   */
  private async extractNoteText(xmlContent: string): Promise<string> {
    try {
   ...
   * 生成纯文本格式
   */
  private generateText(notes: Array<{ slideNumber: number; content: string }>): string {
    const lines: string[] = []
   ...
   * 生成 Markdown 格式
   */
  private generateMarkdown(notes: Array<{ slideNumber: number; content: string }>): string {
    const lines: string[] = []
