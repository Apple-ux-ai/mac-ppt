...
class MemoryMonitor {
   ...
   * 获取峰值内存使用
   */
  getPeakMemory(): MemoryStats | null {
   ...
   * 获取平均内存使用
   */
  getAverageMemory(): MemoryStats | null {
    if (this.samples.length === 0) {
      return null
    }
...
interface StreamProcessingOptions {
  ...
  highWaterMark?: number // 流的高水位标记，默认 16KB
}
...
export class FormatConverter {
  ...
  private imageMagickPath: string | null = null
  private ghostscriptPath: string | null = null
  private timeout: number = 300000 // 5分钟超时
  private memoryMonitor: MemoryMonitor = new MemoryMonitor()
  private readonly largeFileThreshold: number = 50 * 1024 * 1024 // 50MB

  private getBundledLibreOfficePath(): string | null {
    try {
      ...
      for (const executableName of executableNames) {
        candidates.push(
          join(cwd, 'resources', 'LibreOffice', 'program', executableName),
          join(cwd, 'resources', 'libreoffice', 'program', executableName)
        )
      }
      ...
      for (const executableName of executableNames) {
        candidates.push(
          join(distRoot, 'resources', 'LibreOffice', 'program', executableName),
          join(distRoot, 'resources', 'libreoffice', 'program', executableName)
        )
      }
  ...
  private buildLibreOfficeNotFoundMessage(featureName: string): string {
  ...
  async checkImageMagickAvailability(): Promise<boolean> {
    try {
      ...
      possiblePaths.push(
        join(process.cwd(), 'resources', 'ImageMagick-7.1.2-Q16-HDRI', bundledExecutableName)
      )

      // 【优先级 3】dist-electron 同级的 resources 目录（通过 __dirname 推断）
      possiblePaths.push(
        join(projectRootFromDist, 'resources', 'ImageMagick-7.1.2-Q16-HDRI', bundledExecutableName)
      )
      ...
      if (isWindows) {
        try {
          ...
          const candidates = stdout
            ...
            .filter(
              (c) => !c.toLowerCase().includes('system32') && !c.toLowerCase().includes('syswow64')
            )
   ...
   */
  private async getGhostscriptPath(): Promise<string | null> {
    ...
    try {
      ...
      if (resourcesPath) {
        if (isWindows) {
          candidates.push(
            join(resourcesPath, 'gs10.06.0', 'bin', 'gswin64c.exe'),
            join(resourcesPath, 'gs10.06.0', 'bin', 'gswin32c.exe')
          )
        } else {
      ...
      if (isWindows) {
        candidates.push(
          join(cwd, 'resources', 'gs10.06.0', 'bin', 'gswin64c.exe'),
          join(cwd, 'resources', 'gs10.06.0', 'bin', 'gswin32c.exe')
        )
      } else {
   ...
   * @returns 生成的图片文件路径数组
   */
  private async convertPdfToImagesWithImageMagick(
    pdfPath: string,
    outputDir: string,
    format: 'png' | 'jpg'
  ): Promise<string[]> {
    ...
    try {
      ...
      let generatedFiles = files
        .filter((f: string) => f.startsWith('page-') && f.endsWith(`.${format}`))
        .sort()
   ...
   * @returns 生成的图片文件路径数组
   */
  private async convertPptxToImagesDirectly(
    pptxPath: string,
    outputDir: string,
    format: 'png' | 'jpg'
  ): Promise<string[]> {
    try {
      ...
      // @ts-ignore
      const files = fs.readdirSync(outputDir)
      const generatedFile = files.find((f: string) => f.endsWith(`.${format}`))
  ...
  async convertToImages(
    ...
    options?: {
      ...
      width?: number // 输出宽度（保持宽高比）
      height?: number // 输出高度（保持宽高比）
      compressionLevel?: number // PNG 压缩级别 (0-9)，默认 6
    }
  ): Promise<string[]> {
    // 检查输入文件是否存在
    if (!existsSync(inputPath)) {
    ...
    try {
      ...
      // 对于大文件，使用流式处理
      const processedImageFiles: string[] = []
...
// 导出内存监控相关类型和类
export { MemoryMonitor, type MemoryStats, type StreamProcessingOptions }
