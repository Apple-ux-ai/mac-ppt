...
export interface WatermarkOptions {
  ...
  applyToImages?: boolean  // 是否给PPT中的图片添加水印
}
...
export class WatermarkAdder {
   ...
   * @param options 水印选项
   */
  private async addWatermarkToSlideBackground(
    zip: any,
    slideNumber: number,
    options: WatermarkOptions
  ): Promise<void> {
   ...
   * @param options 水印选项
   */
  private async addWatermarkToMaster(zip: any, options: WatermarkOptions): Promise<void> {
    // 查找母版文件
   ...
   */
  private async addWatermarkToImagesInMedia(
    zip: any,
    options: WatermarkOptions,
    ...
    const mediaFiles = zip.file(/\.png$|\.jpg$|\.jpeg$/i).filter((f: any) => f.name.startsWith('ppt/media/'))
    ...
    for (const file of mediaFiles) {
      try {
        ...
        if (options.type === 'text') {
          // 文字水印
          watermarkedImage = await this.addTextWatermarkToImage(
            imageData,
            metadata.width,
            metadata.height,
            options
          ...
          // 图片水印
          watermarkedImage = await this.addImageWatermarkToImage(
            imageData,
            metadata.width,
            metadata.height,
            options
   ...
   * @returns 添加水印后的图片
   */
  private async addTextWatermarkToImage(
    imageData: Buffer,
    width: number,
    height: number,
    options: WatermarkOptions
   ...
   * @returns 添加水印后的图片
   */
  private async addImageWatermarkToImage(
    imageData: Buffer,
    width: number,
    height: number,
    options: WatermarkOptions
   ...
   * @param document PPT文档对象
   */
  private async addWatermarkToSlide(
    zip: any,
    slideNumber: number,
    options: WatermarkOptions,
    document: PptxDocument
   ...
   * @param slideNumber 幻灯片编号
   */
  private async addTextWatermarkAsShape(
    spTree: any,
    options: WatermarkOptions,
    zip: any,
    slideNumber: number
   ...
   * @param slideNumber 幻灯片编号
   */
  private async addImageWatermarkAsShape(
    spTree: any,
    options: WatermarkOptions,
    zip: any,
    slideNumber: number
   ...
   * @param options 水印选项
   */
  private async addTextWatermark(spTree: any, options: WatermarkOptions, zip: any, slideNumber: number): Promise<void> {
    const text = options.text || 'Watermark'
    const fontSize = options.fontSize || 48
    const fontColor = options.fontColor || '000000'
    const opacity = options.opacity || 0.5
   ...
   * @param document PPT文档对象
   */
  private async addWatermarkToImages(
    spTree: any,
    options: WatermarkOptions,
    zip: any,
    slideNumber: number,
   ...
   * @param pictures 图片数组
   */
  private collectAllPictures(element: any, pictures: any[]): void {
    // 直接的图片元素
    if (element['p:pic']) {
   ...
   */
  private addTextWatermarkOnImage(
    spTree: any,
    options: WatermarkOptions,
    imgX: number,
    imgY: number,
   ...
   */
  private async addImageWatermarkOnImage(
    spTree: any,
    options: WatermarkOptions,
    zip: any,
    slideNumber: number,
  ...
  private calculatePosition(
    ...
    slideWidth: number,
    slideHeight: number,
    width: number = 3000000,
    height: number = 1000000
  ): { x: number; y: number } {
    switch (position) {
      case 'top-left':
