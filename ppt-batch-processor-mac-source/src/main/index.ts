...
async function openExternalWithFallback(url: string): Promise<void> {
  const normalizedUrl = normalizeExternalUrl(url)
  try {
    await shell.openExternal(normalizedUrl)
    return
  } catch (error) {
    console.warn('shell.openExternal failed, trying fallback launchers:', error)
  }

  const fallbackCommands: Array<{ command: string; args: string[] }> = []

  if (process.platform === 'linux') {
    fallbackCommands.push(
      { command: 'wslview', args: [normalizedUrl] },
    ...
    if (process.env.WSL_DISTRO_NAME) {
      fallbackCommands.push({
        ...
        args: ['-Command', 'Start-Process', normalizedUrl],
      })
    }
  ...
  for (const fallback of fallbackCommands) {
    try {
      await execFileAsync(fallback.command, fallback.args, { timeout: 15000 })
      return
    } catch (error) {
      lastError = error
    }
  ...
  throw lastError instanceof Error
    ? lastError
    : new Error(`Failed to open external URL: ${normalizedUrl}`)
}
...
function createCallbackServer() {
              ...
              setTimeout(() => {
                try {
                  ...
                  mainWindow?.webContents
                    ...
                    .catch((err: any) => console.error('❌ 执行 JS 失败:', err))
                } catch (error) {
  ...
  callbackServer.on('error', (e: any) => {
    if (e.code === 'EADDRINUSE') {
      console.error(
        `❌ 回调服务器启动失败：端口 3456 已被占用。这通常发生在软件多开或上次退出不彻底时。`
      )
...
app.whenReady().then(async () => {
  ...
  ipcMain.handle('window-maximize', (event) => {
    ...
    return { success: !!win, isMaximized: !!win?.isMaximized() }
  })
  ...
  ipcMain.handle('check-login', async (_event, clientNonce: string) => {
    try {
      ...
      return {
        ...
        isLoggedIn: false,
        ...createLocalizedErrorPayload(
          'main.error.checkLoginStatusFailed',
          error instanceof Error ? error.message : '未知错误'
        ),
      }
    }
  ...
  // 获取用户信息
  ipcMain.handle('get-user-info', async (_event, token: string) => {
    try {
      console.log('=== 获取用户信息 ===')
    ...
    try {
      ...
      if (data.code === 1 && data.data && data.data.length > 0) {
        ...
        return {
          success: false,
          ...createLocalizedErrorPayload(
            'main.error.noAdvertisementAvailable',
            data.msg || '暂无广告'
          ),
        }
      }
      ...
      return {
        success: false,
        ...createLocalizedErrorPayload(
          'main.error.getAdvertisementFailed',
          error instanceof Error ? error.message : '未知错误'
        ),
      }
    }
  ...
  ipcMain.handle('open-external', async (_event, url: string) => {
    try {
      await openExternalWithFallback(url)
      return { success: true, url: normalizeExternalUrl(url) }
    } catch (error) {
      console.error('打开外部链接失败:', error)
      return {
        success: false,
        ...createLocalizedErrorPayload(
          'main.error.openExternalFailed',
          error instanceof Error ? error.message : '未知错误'
        ),
      }
    }
