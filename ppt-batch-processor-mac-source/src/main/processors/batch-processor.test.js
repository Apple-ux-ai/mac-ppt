...
describe('BatchProcessor', () => {
    ...
    describe('pause and resume', () => {
        it('should pause processing', async () => {
            const task = createMockTask();
            task.inputFiles = Array.from({ length: 5 }, (_, i) => createMockFile(`file${i}.pptx`));
            processor.addTask(task);
            const listener = vi.fn();
    ...
    describe('cancel', () => {
        it('should cancel processing', async () => {
            const task = createMockTask();
            task.inputFiles = Array.from({ length: 10 }, (_, i) => createMockFile(`file${i}.pptx`));
            processor.addTask(task);
            const listener = vi.fn();
    ...
    describe('getProgress', () => {
        ...
        it('should calculate percentage as (completedFiles / totalFiles) * 100', async () => {
            const task = createMockTask();
            task.inputFiles = Array.from({ length: 4 }, (_, i) => createMockFile(`file${i}.pptx`));
            processor.addTask(task);
        ...
        it('should reach 100% when all files are processed', async () => {
            const task = createMockTask();
            task.inputFiles = Array.from({ length: 3 }, (_, i) => createMockFile(`file${i}.pptx`));
            processor.addTask(task);
        ...
        it('should maintain thread-safety with concurrent processing', async () => {
            const task = createMockTask();
            task.inputFiles = Array.from({ length: 20 }, (_, i) => createMockFile(`file${i}.pptx`));
            processor.addTask(task);
            const progressSnapshots = [];
            processor.on('progress-update', (progress) => {
                progressSnapshots.push({ ...progress });
    ...
    describe('error handling', () => {
        ...
        it('should emit file-error event on file processing failure', async () => {
            ...
            processor.processFile = vi.fn().mockRejectedValue(new Error('File processing failed'));
            const listener = vi.fn();
    ...
    describe('concurrent processing', () => {
        ...
        it('should respect concurrency limit', async () => {
            ...
            const task = createMockTask();
            task.inputFiles = Array.from({ length: 6 }, (_, i) => createMockFile(`file${i}.pptx`));
            let activeCount = 0;
            let maxActiveCount = 0;
            // Override processFile to track concurrent executions
            const originalProcessFile = customProcessor.processFile.bind(customProcessor);
            customProcessor.processFile = vi.fn().mockImplementation(async (file, task) => {
                activeCount++;
    ...
    describe('memory monitoring', () => {
        it('should track memory usage during processing', async () => {
            ...
            task.inputFiles = Array.from({ length: 10 }, (_, i) => createMockFile(`file${i}.pptx`));
            const memoryUpdates = [];
            customProcessor.on('memory-usage', (info) => {
        ...
        it('should adjust concurrency when memory usage is high', async () => {
            ...
            customProcessor.on('concurrency-decreased', (info) => {
...
// Helper functions
function createMockTask(id = 'test-task') {
    return {
        id,
        name: 'Test Task',
        category: 'content',
...
function createMockFile(name = 'test.pptx') {
    return {
        name,
        path: `/input/${name}`,
        size: 1024 * 1024, // 1MB
