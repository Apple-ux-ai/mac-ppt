...
export class ExcelRulesProcessor {
     ...
     * @returns 规则数组
     */
    async parseExcelRules(rulesPath) {
     ...
     * @returns 处理结果
     */
    async applyRulesToFile(inputPath, outputPath, rulesPath, options) {
        ...
        try {
            ...
            for (let i = 0; i < rules.length; i++) {
                ...
                if (rule.type === 'text-replace' && rule.searchText !== undefined && rule.replaceText !== undefined) {
                    operations = await this.textProcessor.replaceTextInDocument(document, rule.searchText, rule.replaceText, options);
                }
                else if (rule.type === 'delete-content' && rule.deleteContent) {
                    operations = await this.textProcessor.replaceTextInDocument(document, rule.deleteContent, '', options);
                }
     ...
     * @returns 验证结果
     */
    async validateRulesFile(rulesPath) {
        const errors = [];
        const warnings = [];
        try {
            // 检查文件是否存在
