...
export class TextProcessor {
     ...
     */
    async replaceText(inputPath, outputPath, options) {
        ...
        console.log(`         ✓ 文件已打开`);
        // 执行文本替换
        const replacementCount = await this.replaceTextInDocument(document, options.searchText, options.replaceText, options);
     ...
     * @returns 替换的总次数
     */
    async replaceTextInDocument(document, searchText, replaceText, options) {
        ...
        for (const slide of document.slides) {
            ...
            // 使用新的支持正则表达式的替换方法
            const slideReplacements = await this.replaceTextInSlideWithOptions(document, slide.index, searchText, replaceText, options);
     ...
     * @returns 替换次数
     */
    async replaceTextInSlideWithOptions(document, slideIndex, searchText, replaceText, options) {
     ...
     * @returns 匹配的文本元素
     */
    findTextWithOptions(slide, searchText, options) {
     ...
     * @returns 是否成功
     */
    async replaceTextWithOptions(document, element, searchText, replaceText, options) {
     ...
     * @returns 替换结果
     */
    replaceInString(text, searchText, replaceText, options) {
        let result = text;
        let replaced = false;
        let count = 0;
     ...
     * @returns 出现次数
     */
    countOccurrencesWithOptions(text, searchText, options) {
        if (!searchText)
            return 0;
        if (options?.useRegex) {
            try {
     ...
     * @returns 输出文件路径
     */
    generateOutputPath(inputPath, pattern) {
