...
describe('BatchProcessor - Property 2: Error Isolation', () => {
   ...
   * 生成随机错误索引集合（哪些文件会失败）
   */
  const errorIndicesArbitrary = (fileCount: number) => 
    fc.array(
      fc.integer({ min: 0, max: fileCount - 1 }),
      { minLength: 1, maxLength: Math.max(1, Math.floor(fileCount / 2)) }
  ...
  it('edge case: all files fail but processing completes', async () => {
    ...
    // 让所有文件都失败
    ;(processor as any).processFile = vi.fn().mockRejectedValue(new Error('All files fail'))
  ...
  it('edge case: errors in different concurrent batches', async () => {
    ...
    const files: FileInfo[] = Array.from({ length: 8 }, (_, i) => ({
      name: `file${i}.pptx`,
      path: `/input/file${i}.pptx`,
      size: 1024 * (i + 1),
      format: 'pptx',
