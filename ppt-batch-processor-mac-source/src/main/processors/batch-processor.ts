...
interface MemoryConfig {
  maxMemoryUsagePercent: number  // 最大内存使用百分比 (0-1)
  checkInterval: number           // 内存检查间隔 (毫秒)
...
export class BatchProcessor extends EventEmitter {
  private tasks: Array<{ config: TaskConfig, processor: (file: FileInfo) => Promise<ProcessResult> }> = []
  private currentTask: TaskConfig | null = null
  private progress: ProcessProgress | null = null
  ...
  constructor(concurrency?: number, memoryConfig?: Partial<MemoryConfig>) {
    super()
   ...
   */
  addTask(task: TaskConfig, processor?: (file: FileInfo) => Promise<ProcessResult>): void {
    // 验证任务配置
   ...
   * 默认处理函数（用于向后兼容）
   */
  private defaultProcessor = async (file: FileInfo): Promise<ProcessResult> => {
    return {
      inputFile: file.path,
   ...
   * @returns 进度信息
   */
  getProgress(): ProcessProgress | null {
   ...
   * 检查内存使用并动态调整并发数
   */
  private checkMemoryAndAdjust(): void {
    ...
    if (this.currentMemoryUsage > this.memoryConfig.maxMemoryUsagePercent) {
      const newConcurrency = Math.max(
        this.memoryConfig.minConcurrency,
        Math.floor(this.concurrency * 0.75) // 降低 25%
      )
    ...
    // 如果内存使用较低，可以尝试增加并发数
      ...
      const newConcurrency = Math.min(
        this.memoryConfig.maxConcurrency,
        Math.floor(this.concurrency * 1.25) // 增加 25%
      )
   ...
   * @returns 处理结果数组
   */
  private async processTask(task: TaskConfig, processor: (file: FileInfo) => Promise<ProcessResult>): Promise<ProcessResult[]> {
    ...
    try {
      ...
      const filePromises = task.inputFiles.map((file, index) => 
        this.limit(async () => {
          // 检查是否被取消
          if (this.isCancelled) {
            return null
      ...
      // 过滤掉被取消的结果（null）
      results.push(...fileResults.filter((r): r is ProcessResult => r !== null))
   ...
   * @returns 处理结果
   */
  private async processFile(file: FileInfo, task: TaskConfig): Promise<ProcessResult> {
    // TODO: 实现具体的文件处理逻辑
    // 这里将在后续任务中实现不同类型的处理器
    // 目前返回一个占位结果
   ...
   * @returns 输出文件路径
   */
  private generateOutputPath(file: FileInfo, task: TaskConfig): string {
