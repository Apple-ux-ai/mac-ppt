...
describe('BatchProcessor - Property 17: Concurrent Processing Safety', () => {
   ...
   * 生成随机文件列表
   */
  const fileListArbitrary = fc.array(
    fc.record({
      ...
      format: fc.constant('pptx' as const),
   ...
   * 创建模拟任务
   */
  function createMockTask(files: FileInfo[]): TaskConfig {
    return {
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (concurrency, files) => {
          ...
          // 为每个文件创建唯一的处理标记
          const fileMarkers = new Map<string, string>()
          files.forEach(file => {
            fileMarkers.set(file.path, `marker-${file.path}-${Math.random()}`)
          })

          // 模拟文件处理，每个文件返回其唯一标记
          const originalProcessFile = (concurrentProcessor as any).processFile.bind(concurrentProcessor)
          ;(concurrentProcessor as any).processFile = vi.fn().mockImplementation(async (file: FileInfo, task: TaskConfig) => {
            // 模拟一些处理时间
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (concurrency, files) => {
          ...
          concurrentProcessor.on('file-completed', (result: ProcessResult) => {
  ...
  it('edge case: file count equals concurrency', async () => {
    ...
    const files: FileInfo[] = Array.from({ length: concurrency }, (_, i) => ({
      name: `file${i}.pptx`,
  ...
  it('edge case: rapid concurrent task execution', async () => {
    ...
    const files: FileInfo[] = Array.from({ length: 5 }, (_, i) => ({
      name: `file${i}.pptx`,
      path: `/input/file${i}.pptx`,
      size: 1024 * (i + 1),
      format: 'pptx',
 ...
 * 辅助函数：运行任务并返回结果
 */
async function runTask(concurrency: number, files: FileInfo[]): Promise<ProcessResult[]> {
